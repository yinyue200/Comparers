...
namespace UnitTests.Fast
{
    public class Compare_ReverseUnitTests
    {
        ...
        public void SubstitutesCompareDefaultForComparerDefault()
        {
            ...
            Assert.Equal(FastComparerBuilder.For<int>().Default().MakeReverse().ToString(), comparer.ToString());
        ...
        {
            var list = Enumerable.Range(0, 5).ToList();
            list.Sort(ComparerBuilder.For<int>().Default().MakeReverse());
            Assert.Equal(new[] { 4, 3, 2, 1, 0 }, list);
        }
        ...
        {
            var comparer = ComparerBuilder.For<int?>().Default().MakeReverse();
